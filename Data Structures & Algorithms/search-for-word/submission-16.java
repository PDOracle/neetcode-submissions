class Solution {
    ...
    public boolean sol(char[][] board, String word, String curr, int[][] state, int x, int y){
        ...
        boolean left = sol(board, word, newCurr, state, x - 1, y);
        boolean right = sol(board, word, newCurr, state, x + 1, y);
        boolean up = sol(board, word, newCurr, state, x, y - 1);
        boolean down = sol(board, word, newCurr, state, x, y + 1);
        state[y][x] = 0;
